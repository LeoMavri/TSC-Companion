...
const addStats = async (name: string): Promise<void> => {
	...
	$(MEMBER_SELECTOR).each((_index, element) => {
		...
		getTSCSpyOld(userId).then((spy) => {
			...
			const { longTextInterval, longTextEstimate, toolTipText } =
				formatSpyLong(spy);
...
export const FactionWar = new Page({
	...
	shouldRun: async function () {
		return (
			...
			window.location.href.includes("factions.php?step=")
		);
	},
	...
	start: async function () {
		...
		if (
			window.location.href.includes("/war/rank") ||
			window.location.href.includes("/war/raid-")
		) {
			await addStats(this.name);
		}
	},
