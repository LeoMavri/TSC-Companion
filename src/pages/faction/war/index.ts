...
export const FactionWar = new Page({
  ...
  start: async function () {
    xhook.after(async (request, _response) => {
      if (request.url.includes('faction_wars.php?redirect=false&step=getwarusers&')) {
        const user = await waitForElement('div.member.icons.left', 10_000);
        ...
        $('div.member.icons.left').each((_index, element) => {
