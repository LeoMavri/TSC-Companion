...
export const FactionNormal = new Page({
  ...
  shouldRun: async function () {
    return (
      Settings.getToggle(this.name) && window.location.href.includes('factions.php?step=profile')
    );
  },
    ...
    $(memberDiv)
      ...
      .each((_index, member) => {
        ...
        if (infoBox === undefined) {
          Logger.debug(`${this.name}: Failed to find the player's profile box.`, member);
          return;
        }

        const userHref = $(infoBox).find<HTMLAnchorElement>(ID_HREF_SELECTOR)[0];
        ...
        getTSCSpyOld(userId).then(spy => {
          ...
          $(infoBox).after(
            $('<div>').addClass('tsc-faction-spy').text(spyText).attr('title', tooltipText)
          );
        });
      });
  },
